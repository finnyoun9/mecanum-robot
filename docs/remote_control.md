...
> 当前证据边界：历史车端上，NRF24L01 实物收发、K1 一次锁存、四轮开环电机联动、K9 急停与手柄断电失联停车均已在瓷砖地面验收。失联阈值为 250 ms，功能已验证但精确延迟未仪器测量。替换 Blue Pill 已成功烧录；2026-08-29 更换车端 NRF24L01+ 后，快/慢 SPI 都读到正常 `STATUS=0x0E`，模块、3.3 V 供电和 MCU 接线已验证。下一步仅剩手柄开机后的 Auto-ACK 与实际收包复验；在此之前，历史整车遥控验收不能直接等同于新板当前已复验。底盘滚轮采用标准 X 型布局（STM32 端为车头）。
...
> 接收模块只允许接 3.3V。模块旁应并联 10–47 µF（现车使用 220 µF 也可）电解电容，负极接 GND；可再并 0.1 µF 陶瓷。手柄 `Sig=0` 的含义是没有收到 Auto-ACK，不是需要手工配置“配对码”。
