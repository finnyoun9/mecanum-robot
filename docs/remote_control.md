...
> 当前证据边界：历史车端上，NRF24L01 实物收发、K1 一次锁存、四轮开环电机联动、K9 急停与手柄断电失联停车均已在瓷砖地面验收。失联阈值为 250 ms，功能已验证但精确延迟未仪器测量。替换 Blue Pill 已成功烧录，但 2026-08-27 新接的车端 NRF24L01+ 尚未得到有效 SPI 回读：`CSN` 有访问脉冲，`MISO` 非数字响应，手柄 `Sig=0`。因此本页的链路功能不能当作新板当前已复验。底盘滚轮采用标准 X 型布局（STM32 端为车头）。
