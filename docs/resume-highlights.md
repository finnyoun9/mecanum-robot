...
> 证据边界（2026-08-27）：共享协议、运动学、ROS 2 构建、SIL 和 CI 已验证；STM32 引脚映射、四路电机/编码器、单轮 PI、四轮低速闭环基础动作及 NRF24 遥控曾完成真机验证。LD06 与 IMX219/YOLO 已在 Pi 5 上独立实测。Pi↔STM32 UART/真实里程计、完整 FreeRTOS 外设初始化、IMU/ToF、SLAM/Nav2 和长期带载仍未完成。车端 Blue Pill 与 NRF24 当前故障，历史验收不能写成现阶段可直接运行。
...
- 单轮空载阶跃实测参数 `Kp=100, Ki=300, Kd=0`；四轮遥控闭环目标使用前馈 + `Kp=15, Ki=35, Kd=0`
- 支持运行时通过串口 `CMD_PID_TUNE` 调整增益
- 编码器速度反馈：正交编码器 → 边沿计数 → 弧度/秒转换

**简历写法**：
> 实现 **4 路 PI 速度闭环控制**（100 Hz）与抗积分饱和；完成 448 edges/圈编码器标定、单轮三档阶跃调参，以及四轮低速前进/横移/旋转基础验收；长期带载精度仍待测试。
...
| 编码器驱动 | 四路 GPIO EXTI 软件正交解码，A 相双边沿 448 edges/圈 | 面电机控制岗位 |
...
> - 实现 4 路速度 PI 框架（100 Hz，抗积分饱和），完成 448 edges/圈编码器标定、单轮三档阶跃与四轮低速基础动作；长期带载指标待测
