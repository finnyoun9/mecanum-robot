...
### 2026-08-30（四）：首次遥控建图 — ToF 卡死、堵转无解除路径、Pi/遥控器抢控制权

三个真实问题，均已现场处理：

1. **ST-Link 复位后 ToF 卡在同一读数、`error_flags` 仍是 `0x00`**：不是超时（那会置
   `ERR_TOF_TIMEOUT`），是驱动认为读到"有效"数据但值不再变化——大概率是清中断的
   I2C 写在传输层"成功"但传感器没真收到，测距状态机没有真正重启。同一大类问题的
   新表现形式，处理方式不变：断电 3–5 秒重上电。⚠️ 待办：`tof_sensor.c` 目前无法
   识别"数值连续不变=可能卡死"，只认真正的读写失败；要不要加"连续 N 次完全相同
   读数则强制重新 `tof_init()`"的自愈检测，需要权衡静止场景下的真实低噪声读数
   会不会被误判。**已记入待办，本次未做。**
2. **`robot_clear_motor_fault()` 曾经完全没有触发路径**：ToF 触发后允许倒车的新逻辑
   让 `robot_ctrl_loop()` 在 `tof_emergency` 期间也会跑 PID，倒车如果物理上真被卡住
   （贴墙/摩擦力），会触发堵转保护——而堵转锁存此前只能靠重刷/重启解除。已修复：
   新增 `CMD_CLEAR_MOTOR_FAULT`(0x13) 协议命令，接到 `robot_clear_motor_fault()`；
   同时发现手柄（`firmware/remote_controller`）本来就有 12 个物理按键全部接好、
   按键原始值透传（`NRF24L01_TxPacket[5] = KEY`），**K10 完全不用碰手柄固件**，
   车端 `remote_control.h`/`main.c` 加 `REMOTE_KEY_CLEAR_FAULT=10` 直接生效。
3. **Pi 和遥控器抢 `target_w[]`，遥控器突然失控**：`mcr_hardware_interface.cpp`
   的 `write()` 此前无条件每 10ms（100Hz）下发一次 `CMD_VEL_CTRL`，跟遥控器
   `RemoteTask` 的 10Hz packets 共写同一个 STM32 状态、**没有任何仲裁**。空闲时
   Pi 端命令是 0，相当于每 10ms 把遥控器刚写入的摇杆值又清零一次——`robot.launch.py`
   一起来遥控器立刻"失控"。已在 Pi 侧修复（不用重刷 STM32）：`write()` 改为只在
   命令真正变化、或超过 100ms（远小于固件 250ms 看门狗）没发送时才下发，
   平时跟遥控器的 10Hz 节奏打平。⚠️ ~~真正的仲裁机制还没做，这只是止血~~ ——
   **2026-08-30 当天稍后已补上**，见下一节。

三条都是"能用但不完整"的临时处理，不是最终方案，其中第 3 条当天已经补完整。

### 2026-08-30（五）：手柄优先级仲裁 + 建图质量排查（雷达确认干净，怀疑陀螺仪漂移）

**遥控仲裁补完整**：上面第 3 条的止血方案只是把 Pi 的洪泛频率降到跟手柄同台竞争，
仍然"谁的包后到谁赢"，实测重新拉起 `robot.launch.py` 后手柄依然会间歇性发飘、
不跟手。补上真正的优先级：`robot_state_t` 新增 `remote_active`，`RemoteTask`
每收到一个包（不论 K1 开关）都调用新增的 `robot_set_remote_active()` 刷新；
`robot_handle_command()` 的 `CMD_VEL_CTRL` 分支在 `remote_active` 为真时**跳过**
`target_w[]` 赋值，只让手柄自己写的值生效，但 deadman 恢复判断照常跑。
`remote_active` 自带 250ms 超时（复用 `COMM_WATCHDOG_MS` 量级）——手柄没关 K1
就断电/失联的话，Pi 不会被永久锁死在一个已经不存在的链路后面。真机验证：只按
复位（不用整车断电）就生效，操控明显"丝滑多了"（用户原话）。

**四轮标定的乌龙**：一开始怀疑 RR 电机之前的"偏慢+咔嚓"是四轮 PID/机械不一致，
单独测出过 47% 的轮间差距、还带咔嚓声。反复排查后发现是**链路降级**惹的祸——
同一套代码链路干净时复测，2.5 rad/s 下四轮差距只有 **1.4%**，0.8 rad/s 低速也只有
**8.5%**，`四轮一起`跟`单轮孤立`结果一致。教训记一遍：**每次断电重上电后，第一步
必须先跑 `tools/link_check.py` 确认链路干净，再做任何别的判断**——今天好几次
"看起来是这个部件坏了"的假警报，根子都是没先查链路。新增 `tools/wheel_calibration.py`
做单轮孤立标定（复用 `link_check.py`/`drive_check.py` 的帧解析）。

**建图质量：修完所有已知 bug，图依然不理想**。链路稳定、四轮标定正常、遥控仲裁
补完整之后，重新跑了一次完整绕圈建图——**结果比之前两次更差**，扇贝状边界和放射状
毛刺更多，不是更好。排除法：
- 静止里程计测试（1.6m 直线、原地 360°）数值良好，但**这两个测试时间短、动作单一
  （纯直线或纯旋转）**，跟"绕一整圈、多次转弯、有加减速"的真实建图轨迹完全不是
  一回事——短测试很可能低估了误差累积。
- **雷达本身排除**：车静止时连续采 8 帧 `/scan`，453 个点里 435 个有效，
  逐点标准差中位数 **8.1mm**，非常稳定，不是雷达噪声的问题。
- 剩下最大嫌疑还是回到最早发现的那条：**静止时陀螺仪 yaw 会自己漂（约 0.14°/秒）**。
  绕一圈建图的时长和转弯次数远超短测试，这个漂移加上转弯时的真实轮子打滑，
  累积起来足够让扫描配准失败成这样。

**下一步（未做，留到下次专门开 session）**：
- 检查/重做 AHRS 的陀螺仪零偏校准（开机静置一段时间估算零偏再启用积分）
- 检查 EKF (`ekf.yaml`) 对 IMU yaw 和轮式里程计 yaw 的融合权重，
  轮式里程计的转角可能比陀螺仪更可信，现在的协方差配置未必对
- 如有需要再调 `slam_toolbox_params.yaml` 的扫描匹配容差

### 后续优化：电池电压采样与 OLED 电量显示（已预研，延期启用）

> **不属于当前 M5 阻塞项。** 现阶段保持原接线和现有非电池版固件，待主体导航/建图稳定后再安排改线、烧录与万用表校准。

- 增加 PA4 / ADC1_IN4 的 3S 电池采样：100kΩ/27kΩ 分压，10Hz 采样并做 1/8 IIR 滤波；整数换算电压并用分段曲线估算百分比。
- PA4 原本占用作 FL AIN1，因此 `BATTERY=1` 构建把该方向线条件式迁移到 PB9；默认构建仍沿用 PA4，不影响当前接线和已刷固件。
- OLED 顶栏显示百分比，状态行显示电压；里程计反馈中的 `battery_pct` 不再固定为 0。
- 主机测试 12/12 通过；默认 ARM 固件为 text/data/bss `31744/360/15184`，电池版为 `32356/360/15192`，均通过 `-Wall -Werror` 构建。
- **暂不烧录、不改线**：以后启用时，必须先把 FL AIN1 从 PA4 改到 PB9，并按 `docs/wiring.md` 接好分压与 100nF 电容，再刷 `BATTERY=1` 固件做万用表对照校准。
