...
当前硬件由真机调试端占用（M3 基础动作已验收；**3S 电池供电**给电机总线、MCU 由 ST-Link 3.3 V 供电）。
...
- **M3 四轮闭环**：基础动作已验收；尚未做 30 分钟连续运行、量化的落地带载速度/跟踪误差、
  以及最终 M3 镜像上的 K9 与 250 ms 失联停车复验。四轮电池供电扫描的差异为 4.9%，RR
