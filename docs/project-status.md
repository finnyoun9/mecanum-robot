...
3. **Pi 和遥控器抢 `target_w[]`，遥控器突然失控**：`mcr_hardware_interface.cpp`
   ...
   平时跟遥控器的 10Hz 节奏打平。⚠️ **真正的仲裁机制（比如手柄 K1 使能时忽略 Pi
   指令）还没做，这只是止血**：如果 Pi 端在 Nav2 之类场景主动持续下发非零指令，
   同时手柄也在发，现在谁的包"最后到"还是会赢。已记入待办。

三条都是"能用但不完整"的临时处理，不是最终方案，都已记入待办列表，之后统一处理。
