...
- **NRF24 遥控链路（2026-08-25 实测）**：车端 `remote_drive` 目标 + 手柄 `remote_controller` 固件均已烧录。
  ...
  - ⚠️ **未验证**：K1 使能后的运动方向、K9 急停、失联 100 ms 停机——见下方"当前卡点"。
...
**现象**：手柄 OLED 不亮；小车轮子只动一下就停（这是失联保护正常工作的表现——收到零星包后 100 ms 内没有后续包）。
...
**待验证的七步**（底盘抬起做完再落地）：不按 K1 不动 → 按 K1 后左摇杆前推四轮正转 → 左摇杆左右横移 → 右摇杆左右原地旋转 → K9 急停 → **关掉手柄电源应在 100 ms 内自动停**。最后一条最关键。
