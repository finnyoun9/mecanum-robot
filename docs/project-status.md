...
- **NRF24 遥控链路（2026-08-25 实测）**：车端 `remote_drive` 目标 + 手柄 `remote_controller` 固件均已烧录。
  ...
  - **代码级诊断（2026-08-25，待真机复验）**：原车端 100 ms 超时与实测
    9.96 Hz（平均 100.4 ms）发送周期竞争，会在正常相邻包之间清掉 K1 锁存，
    正是“动一下就停、每次都要重按 K1”的直接原因。已改为 250 ms；K1 锁存的
    “K1 一次 + 后续无键摇杆包仍保持使能”主机回归测试通过。
  - ⚠️ **仍待真机验证**：K1 使能后的连续运动/方向、K9 急停、手柄断电后约 250 ms 停机。
...
- **遥控 7 步验证**（方向/急停/失联）尚未跑完，卡在手柄供电，见"当前卡点"。
...
遥控链路本身已证明可用（249 包 / 9.96 Hz），但**七步验证没跑完**，卡在硬件供电上：

**已定位的软件问题（2026-08-25，尚未回刷真机）**：车端原来把 10 Hz（实测
9.96 Hz）的正常相邻包间隔当作 100 ms 超时；因此会清掉 K1 锁存，造成“只动一下，
每次摇杆前都要重按 K1”。`remote_drive` 已改为 250 ms 超时。这个改动不替代下面的
供电排查：OLED/USB 供电仍需独立确认。

**此前现象**：手柄 OLED 不亮；小车轮子只动一下就停。

**最可能的原因**：**USB 供电预算超了**。当时 Mac 上插着两个 ST-Link，一个给小车（Blue Pill + 4 编码器 + 2 块 TB6612 逻辑）供电、一个给手柄（MCU + OLED + NRF24）供电。期间**小车的 ST-Link 直接从 USB 掉线**（`Couldn't find any ST-Link devices`），两个症状同时出现，指向同一个原因。刚烧完手柄时链路是好的（249 包），那时负载还没全接上。

**下一步（需要真机，属硬件侧）**：
1. 两边都改用**自己的电池**供电，ST-Link 烧完即拔——遥控测试本来就要求小车脱线。
2. 用 OLED 亮不亮当手柄的通电指示。
3. **一个尚未回答的判别问题**：手柄 OLED **在刚烧录完那次亮过没有**？
   - 亮过 → 纯供电问题，独立供电即可解决。
   - 从没亮过 → 本仓库移植版的 OLED 引脚（PB8=SCL / PB9=SDA 软件 I2C）可能与实际遥控器硬件不符，需对照 `robot-vacuum-lab/平衡车项目资料/` 的江科大原版核对。注意 **NRF24 部分是好的**（249 包已证明），只有 OLED 存疑。

**待验证的七步**（底盘抬起做完再落地）：不按 K1 不动 → **只按一次** K1 后，连续推/松/再推左摇杆仍可运动 → 左摇杆前推四轮正转 → 左摇杆左右横移 → 右摇杆左右原地旋转 → K9 急停 → **关掉手柄电源应在约 250 ms 内自动停**。最后一条最关键。
