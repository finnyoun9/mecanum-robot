...
bool mpu6050_init(void) {
    uint8_t who_am_i = 0;
    /* HAL_I2C_Mem_Read(&hi2c1, MPU6050_I2C_ADDR << 1, MPU6050_REG_WHO_AM_I,
    ...
    if (who_am_i != MPU6050_WHOAMI_VAL) {
         ...
         * Some MPU6050 clone boards report 0x72/0x98 instead of the
         * datasheet's 0x68 — relax this check if a genuine clone fails it. */
    }
    ...
    reg = 0x01;
    /* HAL_I2C_Mem_Write(&hi2c1, MPU6050_I2C_ADDR << 1, MPU6050_REG_PWR_MGMT_1,
                          I2C_MEMADD_SIZE_8BIT, &reg, 1, HAL_MAX_DELAY); */

    /* Enable all accel/gyro axes (clear standby bits) */
    reg = 0x00;
    /* HAL_I2C_Mem_Write(&hi2c1, MPU6050_I2C_ADDR << 1, MPU6050_REG_PWR_MGMT_2,
                          I2C_MEMADD_SIZE_8BIT, &reg, 1, HAL_MAX_DELAY); */

    /* DLPF_CFG=3 -> ~44Hz bandwidth (gyro AND accel share this on 6050),
     * internal sample rate becomes 1kHz */
    reg = 0x03;
    /* HAL_I2C_Mem_Write(&hi2c1, MPU6050_I2C_ADDR << 1, MPU6050_REG_CONFIG,
                          I2C_MEMADD_SIZE_8BIT, &reg, 1, HAL_MAX_DELAY); */

    /* Sample rate = 1kHz / (1 + SMPLRT_DIV) -> divider=9 gives 100Hz */
    reg = 9;
    /* HAL_I2C_Mem_Write(&hi2c1, MPU6050_I2C_ADDR << 1, MPU6050_REG_SMPLRT_DIV,
                          I2C_MEMADD_SIZE_8BIT, &reg, 1, HAL_MAX_DELAY); */

    /* Gyro full-scale: +/-500 dps */
    reg = MPU6050_GYRO_FS_500DPS;
    /* HAL_I2C_Mem_Write(&hi2c1, MPU6050_I2C_ADDR << 1, MPU6050_REG_GYRO_CONFIG,
                          I2C_MEMADD_SIZE_8BIT, &reg, 1, HAL_MAX_DELAY); */

    /* Accel full-scale: +/-4g */
    reg = MPU6050_ACCEL_FS_4G;
    /* HAL_I2C_Mem_Write(&hi2c1, MPU6050_I2C_ADDR << 1, MPU6050_REG_ACCEL_CONFIG,
                          I2C_MEMADD_SIZE_8BIT, &reg, 1, HAL_MAX_DELAY); */
...
void mpu6050_read_raw(int16_t accel[3], int16_t gyro[3]) {
    ...
    /* HAL_I2C_Mem_Read(&hi2c1, MPU6050_I2C_ADDR << 1, MPU6050_REG_ACCEL_XOUT_H,
